...
func multi_cast(index: int, caster: CharacterBody2D, location: Vector2):
	...
	if spell_sequence[spell_sequence.size() - 1] in spells:
		spells[spell_sequence[spell_sequence.size() - 1]].cast(caster, location, spell_power, duration_boost, extra_mod)
		clear_buffs()
		spell_sequence.clear()
		return true
...
func _on_spell_cast(spell_name: String, caster: CharacterBody2D, location: Vector2):
	if spell_name in spells:
		if spells[spell_name].is_castable:
			if spell_sequence.size() == 0:
				spells[spell_name].cast(caster, location, spell_power, duration_boost, extra_mod)
				print("Spell Cast: " + spell_name)
				print(location)
			else:
				spell_sequence.push_back(spell_name)
				var end_of_buffs_index = apply_buffs()
				multi_cast(end_of_buffs_index, caster, location)
		else:
